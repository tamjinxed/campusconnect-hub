  ...
  return (
    <div className="flex flex-col h-[calc(100vh-12rem)]">
      ...
      <div className="flex gap-2">
        <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSend()} placeholder="Ask anything..." className="rounded-xl" />
        <Button size="icon" onClick={handleSend} className="rounded-xl shrink-0"><Send size={16} /></Button>
      </div>
    </div>
  );
};

// Teacher Chat Page — classrooms only with create option
const TeacherChatPage = () => {
  ...
  const handleCreate = () => {
    if (!className.trim()) { toast.error("Enter a classroom name"); return; }
...
// Student Chat Page — classroom, groups, AI
  ...
  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
      <TabsList className="w-full rounded-xl bg-muted/60 p-1 mb-4">
        <TabsTrigger value="classroom" className="flex-1 rounded-lg text-xs font-semibold">Classroom</TabsTrigger>
        <TabsTrigger value="chat" className="flex-1 rounded-lg text-xs font-semibold">Groups</TabsTrigger>
        <TabsTrigger value="ai" className="flex-1 rounded-lg text-xs font-semibold"><Bot size={13} className="mr-1" /> AI</TabsTrigger>
      </TabsList>
      <TabsContent value="classroom">
        <div className="bg-card border border-border rounded-2xl p-3 mb-4">
          ...
          <div className="flex gap-2">
            <Input value={joinCode} onChange={(e) => setJoinCode(e.target.value)} placeholder="Enter classroom code" className="rounded-xl text-sm" />
            <Button size="sm" onClick={handleJoin} className="rounded-xl text-xs shrink-0">Join</Button>
          </div>
        </div>
        <ClassroomList onSelect={setSelectedClassroom} />
      </TabsContent>
      <TabsContent value="chat"><ChatList onSelectGroup={setSelectedGroup} /></TabsContent>
      <TabsContent value="ai"><AIChatView /></TabsContent>
    </Tabs>
  );
};
